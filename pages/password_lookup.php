        ...
        if ($stmt = $conn->prepare("SELECT `id`,`displayname` FROM `user` WHERE email = ?")) {
            ...
            if ($result->num_rows > 1) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    $sendMail = sendEmail($email, "สวัสดี " . $row['displayname'] . "! คุณได้ทำการส่งคำร้องขอรีเซ็ตรหัสผ่านเพื่อเข้าใช้งานเว็บไซต์ grader.ga", "http://lca.pondja.com/static/functions/resetpassword/resetpassword.html", $var);
                    if ($sendMail) {
